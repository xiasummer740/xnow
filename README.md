...
* **📦 大厂级数据灾备引擎**：内置独立 CLI 工具，一键生成冷冻级 `.sql.gz` 极小体积快照。支持无损覆盖还原，重装跑路无缝衔接。
...
* 自动创建数据库、生成安全密钥。
* 自动编译前端 Vite 代码、配置 Nginx 域名绑定与反向代理。
...
本系统内置独立的可视化数据库管理引擎。无论您是需要日常备份，还是服务器重装后的数据迁移，都能轻松应对。

**一键召唤灾备面板：**
~~~bash
bash /var/www/xnow/tools/xnow_db_manager.sh
~~~
* **常规备份**：运行面板选择 `1`，系统将在 `/root/xnow_backups/` 生成极小体积的 `.sql.gz` 快照，请用 SFTP 妥善下载保存到本地。
* **重装恢复**：在新服务器运行完 `install.sh` 后，将备份包传回 `/root/xnow_backups/` 目录。运行面板选择 `2`，一秒满血复活所有核心数据！
