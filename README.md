...
│   │   │   ├── AdminBackup.vue # 【灾备中心】可视化的全自动数据备份与极致覆盖还原 GUI 面板
...
└── server/                 # 🔴 后端 Node.js + Express 项目目录 (ES6 Modules)
    ...
    │   │   ├── admin.js    # 管理员特权控制台 (一键封禁、可视化灾备恢复接口)
...
* **📦 可视化自动化数据灾备系统**：支持在前端可视化设置定时备份频率（如每 0.5 小时），防失联下载，且支持新部署时直接上传历史备份包实现秒级强制复活！
...
* 设置自动化备份频率（如 0.5 小时一次）
* 随时手动创建快照并下载
* 上传历史 `.sql.gz` 快照包，一键还原覆盖
