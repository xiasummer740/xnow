...
└── server/                 # 🔴 后端 Node.js + Express 项目目录 (ES6 Modules)
    ...
    │   │   ├── auth.js     # 身份验证集群 (前置防重复验证码拦截 + 影子账户引擎)
    ...
    │   │   ├── email.js    # 📧 经典企业级白底蓝标 HTML 邮件引擎 (全维度模糊匹配配置)
...
* **📧 经典动态邮件引擎**：内置 `Nodemailer` 核心，动态读取数据库 SMTP 配置，发送典雅、清爽的企业级白底蓝标验证码邮件。
* **🛡️ 生产级前置防克隆系统**：在请求发送验证码的瞬间激活雷达，精准拦截已被注册的邮箱，彻底杜绝资源浪费与重复注册。
