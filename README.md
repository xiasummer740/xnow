...
* **🕸️ 骨灰级全球 SEO 矩阵**：内置极限关键词（tk, ins, youtube, tg, 涨粉, 播放量等）、Open Graph 社交图谱及 JSON-LD 结构化数据，霸榜 Google 与百度，实现全球流量截流。
...
* **🚀 裸机级一键部署引擎**：小白专属 `install.sh`。自动装配 Node、MySQL、Nginx、PM2，双轨 SSL 容灾策略（Let's Encrypt → 自签证书自动降级），结尾健康检查确认服务正常。注：依赖包的漏洞警告（`npm audit`）不影响核心功能，可忽略。
...
按照屏幕提示输入您的 **网站域名** 和自定义的 **数据库密码**。随后脚本全自动执行：

| 步骤 | 操作 | 说明 |
|------|------|------|
| 1/6 | 安装系统依赖 | apt 安装 Nginx、MySQL/MariaDB、curl 等 |
| 2/6 | Node.js 18 + PM2 | 不存在则自动安装，已安装则跳过 |
| 3/6 | 配置 MySQL | 创建数据库 xnow_db，设置密码（自动适配 mysql/mariadb 服务名） |
| 4/6 | 拉取源码 | 已有代码则 `git pull` 更新 |
| 5/6 | 构建后端 | 初始化 `.env`（保留上次 JWT），npm 安装，PM2 开机自启 |
| 6/6 | 构建前端 | npm 安装 + Vite 编译 |

然后自动配置 SSL 证书（Let's Encrypt → 自签证书降级机制）和 Nginx 站点。

> **💡 脚本可重复执行**：重跑不会丢失数据，`JWT_SECRET` 自动保留避免登录失效，旧 `.env` 自动备份。
>
> 执行完毕后会进行**健康检查**，自动校验后端 API 和 Nginx 是否正常，有问题会提示。

### 3. 💡 首次初始化 (系统引导)
部署完成后，请在浏览器打开您的域名：
1.  直接在前台**注册页使用您的手机号完成首次注册**。
2.  ⚡️ 系统无任何管理员时，**第一个注册的账号自动成为全站管理员**，进入前台【管理密室】配置全局参数、支付网关、TG 机器人等。
3.  之后新增注册一律为普通用户（黄金用户）。
> 系统不内置任何影子超管账号/口令，管理员身份只由首个注册账号获得。
