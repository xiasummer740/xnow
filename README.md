...
~~~text
/xnow/
├── install.sh              # 🟢 【入口】裸机引导部署引擎 (全自动装环境、建库、编译、Nginx、SSL)
...
└── server/                 # 🔴 后端 Node.js + Express 项目目录 (ES6 Modules)
    ...
    │   │   ├── admin.js    # 管理员特权控制台 (一键封禁、加扣款、全局倍率)
...
* **🛡️ SaaS 级无痕冷启动**：首次部署后，输入 `admin` / `admin123` 虚空登入系统。首位真实注册用户自动加冕为「至尊管理员」并彻底自毁后门。
* **🚀 裸机级一键部署引擎**：小白专属 `install.sh`。只需输入域名与密码，脚本自动装配 Node、MySQL、Nginx 并申请 SSL 证书，解放双手。
* **🤖 全场景 Telegram 超强引擎**：异步非阻塞式推送。新用户注册、资金入账、上下级返佣、违规封禁等全链路实时监控。
* **💸 暴利裂变分销中枢**：内置全局流量锁客系统与阶梯式（普通5% / 代理10%）佣金自动结算大脑，自带抗拦截 CSS 极客邮件营销模板。
* **💳 多通道极速支付网关**：内置 BufPay 与 Cryptomus 聚合支付。智能计算 5% 通道费并实现隐写本金入账。
* **🎨 顶级交互体验**：纯 CSS 手绘 3D 卡哇伊客服交互气泡、USDT 充值全屏响应式 Lightbox（灯箱）。
* **🔒 企业级安全 WAF 铠甲**：深度集成 Helmet 安全头、防 XSS 注入、基于真实穿透 IP 的 15分钟/30次 超强 CC 防爆破限流锁，以及后台可视化数字护城河（IP 物理级拦截）与账号一键封杀系统。
...
* 自动创建数据库、生成安全密钥。
* 自动编译前端 Vite 代码、配置 Nginx 域名绑定。
* 自动向 Let's Encrypt 申请 HTTPS 证书！
...
4.  ⚡️ **系统判定机制生效**：您的新账号将被永久赋权为 `super_admin`（UID为1）。此时，`admin` 影子后门瞬间永久物理销毁，保证系统绝对纯净安全！
