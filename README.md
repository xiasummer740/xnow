...
└── server/                 # 🔴 后端 Node.js + Express 项目目录 (ES6 Modules)
    ...
    │   │   ├── auth.js     # 身份验证集群 (内置影子账户冷启动机制与动态邮件分发)
    ...
    │   │   ├── email.js    # 📧 极客风动态 HTML 邮件发送引擎 (全维度模糊匹配配置)
...
* **📧 极客风动态邮件引擎**：内置 `Nodemailer` 核心，动态读取数据库 SMTP 配置，支持全维度参数模糊匹配，发送抗拦截的黑金炫酷 HTML 验证码邮件。
* **🛡️ 生产级防克隆系统**：通过 API 层的严格数据清洗，彻底斩断大小写和空格绕过，杜绝手机号与邮箱的重复注册行为。
