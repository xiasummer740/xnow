<template>
  <div class="vpn-page min-h-full space-y-5" style="max-width:1100px;margin:0 auto;padding:0 1rem 2rem">
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:0.75rem">
      <div class="xui-card" style="padding:1rem;text-align:center"><div style="font-size:1.5rem;font-weight:900;color:var(--xui-primary)">{{ servers.length }}</div><div style="font-size:0.65rem;color:var(--xui-text-dim);margin-top:0.25rem;text-transform:uppercase;letter-spacing:0.05em">节点</div></div>
      <div class="xui-card" style="padding:1rem;text-align:center"><div style="font-size:1.5rem;font-weight:900;color:#0891b2">{{ clients.length }}</div><div style="font-size:0.65rem;color:var(--xui-text-dim);margin-top:0.25rem;text-transform:uppercase;letter-spacing:0.05em">订单</div></div>
      <div class="xui-card" style="padding:1rem;text-align:center"><div style="font-size:1.5rem;font-weight:900;color:#d97706">{{ activeCount }}</div><div style="font-size:0.65rem;color:var(--xui-text-dim);margin-top:0.25rem;text-transform:uppercase;letter-spacing:0.05em">活跃</div></div>
      <div class="xui-card" style="padding:1rem;text-align:center"><div style="font-size:1.5rem;font-weight:900;color:#7c3aed">{{ totalSold }}</div><div style="font-size:0.65rem;color:var(--xui-text-dim);margin-top:0.25rem;text-transform:uppercase;letter-spacing:0.05em">已售GB</div></div>
    </div>
...
const deleteClient = async (c) => { if (!await uiStore.showConfirm(`确认删除「${c.email}」？也会从 XX-UI 删除。`)) return; try { await fetch(`/api/vpn/admin/client/${c.id}`, { method: 'DELETE', headers: getHeaders() }); fetchData(); } catch (e) { uiStore.showToast('删除失败', 'error'); } };
