...
const submitDeleteUser = async () => {
    ...
    try {
        const res = await fetch('/api/admin/user/delete', { 
            ...
            body: JSON.stringify({ userId: deleteUserModal.value.userId }) 
        });
        const json = await res.json();
        if (json.status === 'success') { 
            ui.showToast('账号及所有关联流水已被系统彻底物理销毁！', 'success'); 
...
<template>
    <div class="max-w-7xl mx-auto space-y-6 pb-20 relative z-10">
        ...
        <div class="bg-slate-800/80 border border-slate-700 p-6 md:p-8 rounded-3xl shadow-xl flex flex-col gap-6 w-full">
            <div class="flex items-center space-x-4 border-b border-slate-700 pb-4"><div class="w-12 h-12 bg-blue-500/20 rounded-full flex items-center justify-center text-blue-400 text-2xl shrink-0">⚡</div><div><h3 class="text-white font-bold text-lg md:text-xl tracking-wide">核心系统连接配置 (System Core)</h3><p class="text-xs text-slate-400 mt-1">第三方服务参数配置中心 (管理员填入后生效，清空则停止工作)</p></div></div>
            ...
            <button @click="saveConfig" class="w-full mt-4 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-black text-lg py-4 rounded-xl transition shadow-[0_10px_30px_rgba(79,70,229,0.3)] transform hover:-translate-y-1 tracking-wider">保存系统配置</button>
        </div>
        ...
        <div v-if="deleteUserModal.show" class="fixed inset-0 z-[10007] flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-fade-in">
          <div class="bg-slate-900 border-2 border-red-500/50 p-8 rounded-3xl shadow-[0_0_80px_rgba(239,68,68,0.4)] max-w-md w-full relative transform transition-all scale-100">
            ...
            <p class="text-slate-300 text-sm mb-6 text-center leading-relaxed">
              您正在强制抹除用户 <span class="text-amber-400 font-bold font-mono">{{ deleteUserModal.phone }}</span> (UID: {{ deleteUserModal.userId }})。<br><span class="text-red-400 font-bold">此账号及其产生的所有资金记录将被永久物理销毁！</span>
            </p>
            <div class="space-y-4 mb-8">
              <div class="bg-red-500/10 border border-red-500/30 p-4 rounded-xl text-center">
                <span class="text-xs text-slate-400 block mb-2">为防止误操作，请输入下方红字确认：</span>
                <span class="text-lg font-black text-red-500 tracking-widest select-none cursor-not-allowed">执行死刑</span>
              </div>
