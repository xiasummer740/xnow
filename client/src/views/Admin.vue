...
// === 服务端分页：交易流水 ===
const txPage = ref(1); const txLimit = ref(20); const txSearch = ref(''); const txType = ref(''); const txDateFrom = ref(''); const txDateTo = ref('');
const txData = ref({ items: [], total: 0 }); const txLoading = ref(false);
const totalTxPage = computed(() => Math.ceil(txData.value.total / txLimit.value) || 1);
const fetchTx = async () => {
  txLoading.value = true;
  try {
    const params = new URLSearchParams({ page: txPage.value, limit: txLimit.value });
    if (txSearch.value) params.set('search', txSearch.value);
    if (txType.value) params.set('type', txType.value);
    if (txDateFrom.value) params.set('date_from', txDateFrom.value);
    if (txDateTo.value) params.set('date_to', txDateTo.value);
    const res = await fetch(`/api/admin/transactions?${params}`, { headers: { 'Authorization': `Bearer ${userStore.token}` } });
    const j = await res.json();
    if (j.status === 'success') txData.value = j.data;
  } catch (e) { ui.showToast('交易流水加载失败', 'error'); } finally { txLoading.value = false; }
};
watch([txPage, txLimit, txSearch, txType, txDateFrom, txDateTo], () => { if (txPage.value > 1 && (txSearch.value || txType.value || txDateFrom.value)) txPage.value = 1; fetchTx(); }, { immediate: true });

// === 服务端分页：用户 ===
const userPage = ref(1); const userLimit = ref(20); const userSearch = ref(''); const userRole = ref(''); const userDateFrom = ref(''); const userDateTo = ref('');
const userData = ref({ items: [], total: 0 }); const userLoading = ref(false);
const totalUserPage = computed(() => Math.ceil(userData.value.total / userLimit.value) || 1);
const fetchUsers = async () => {
  userLoading.value = true;
  try {
    const params = new URLSearchParams({ page: userPage.value, limit: userLimit.value });
    if (userSearch.value) params.set('search', userSearch.value);
    if (userRole.value) params.set('role', userRole.value);
    if (userDateFrom.value) params.set('date_from', userDateFrom.value);
    if (userDateTo.value) params.set('date_to', userDateTo.value);
    const res = await fetch(`/api/admin/users?${params}`, { headers: { 'Authorization': `Bearer ${userStore.token}` } });
    const j = await res.json();
    if (j.status === 'success') { userData.value = j.data; attachGeo(userData.value.items); }
  } catch (e) { ui.showToast('用户列表加载失败', 'error'); } finally { userLoading.value = false; }
};
watch([userPage, userLimit, userSearch, userRole, userDateFrom, userDateTo], () => { if (userPage.value > 1) userPage.value = 1; fetchUsers(); }, { immediate: true });

// === 服务端分页：订单 ===
const orderPage = ref(1); const orderLimit = ref(20); const orderSearch = ref(''); const orderStatus = ref(''); const orderDateFrom = ref(''); const orderDateTo = ref('');
const orderData = ref({ items: [], total: 0 }); const orderLoading = ref(false);
const totalOrderPage = computed(() => Math.ceil(orderData.value.total / orderLimit.value) || 1);
const fetchOrders = async () => {
  orderLoading.value = true;
  try {
    const params = new URLSearchParams({ page: orderPage.value, limit: orderLimit.value });
    ...
    const res = await fetch(`/api/admin/orders?${params}`, { headers: { 'Authorization': `Bearer ${userStore.token}` } });
    const j = await res.json();
    if (j.status === 'success') orderData.value = j.data;
  } catch (e) { ui.showToast('订单列表加载失败', 'error'); } finally { orderLoading.value = false; }
};
...
// 批量查本页用户 IP 的地区，回填到行对象（走 /geo/batch 一次搞定，避免逐个外呼）
const attachGeo = async (items) => {
  if (!items || !items.length) return;
  const ips = [];
  items.forEach((u) => { [u.register_ip, u.last_login_ip].forEach((ip) => { if (ip && !['127.0.0.1','::1','localhost'].includes(ip) && !ips.includes(ip)) ips.push(ip); }); });
  if (!ips.length) return;
  try {
    const r = await fetch('/api/admin/geo/batch', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${userStore.token}` }, body: JSON.stringify({ ips }) });
    const j = await r.json();
    if (j.status !== 'success') return;
    items.forEach((u) => {
      const rg = j.data[u.register_ip]; const lg = j.data[u.last_login_ip];
      if (rg) u.registerLoc = rg.city ? `${rg.country} ${rg.city}` : rg.country;
      if (lg) u.loginLoc = lg.city ? `${lg.country} ${lg.city}` : lg.country;
    });
  } catch {}
};
...
const exportCSV = (endpoint, params) => {
  ...
  window.open(`/api/admin/${endpoint}/export?${p}`);
};
        ...
        <div v-if="userDetail.show && userDetail.data" class="fixed inset-0 z-[10004] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-fade-in">
          <div class="bg-slate-900 border border-slate-700 p-6 rounded-3xl shadow-[0_20px_60px_rgba(0,0,0,0.8)] max-w-3xl w-full max-h-[90vh] overflow-y-auto custom-scrollbar relative">
            ...
            <div class="grid grid-cols-3 gap-3 text-sm mb-4">
              ...
              <div class="bg-slate-800/50 rounded-xl p-3 border border-slate-700/50"><span class="text-xs text-slate-400 block mb-1">角色</span><span class="text-amber-400 font-bold">{{ ({ super_admin: app.t('super_admin_badge'), admin: app.t('admin_badge'), agent: app.t('agent_badge') || '👑 至尊代理', gold: app.t('gold_badge') })[userDetail.data.role] || app.t('gold_badge') }}</span></div>
              <div class="bg-slate-800/50 rounded-xl p-3 border border-slate-700/50"><span class="text-xs text-slate-400 block mb-1">余额</span><span v-if="['admin','super_admin'].includes(userDetail.data.role)" class="text-green-400 font-bold font-mono flex items-center"><span class="text-[9px] bg-green-500/20 text-green-400 px-1.5 py-0.5 rounded-full border border-green-500/30 mr-2">API</span>{{ data.upstreamBalance?.balance || '查询中...' }}</span><span v-else class="text-green-400 font-bold font-mono">{{ app.formatMoney(userDetail.data.balance) }}</span></div>
