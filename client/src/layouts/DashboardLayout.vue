<template>
  <div class="h-[100dvh] w-full flex bg-transparent relative overflow-hidden">
    ...
    <aside :class="['fixed md:relative z-[9999] md:z-50 h-full w-60 bg-slate-900/95 border-r border-slate-800 flex flex-col backdrop-blur-xl transition-transform duration-300 ease-in-out', isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0']">
      <div class="h-16 flex items-center px-5 border-b border-slate-800">
        <a href="/" class="flex items-center w-full hover:opacity-80 transition" title="返回首页">
          <img :src="appStore.siteLogo || '/logo.png'" onerror="this.onerror=null; this.src='/logo.png';" class="h-8 w-8 object-contain mr-2.5" alt="Logo">
          <span class="text-lg font-black tracking-wide truncate bg-gradient-to-r from-amber-300 to-amber-500 bg-clip-text text-transparent">{{ appStore.siteName }}</span>
        </a>
      ...
      <div class="flex flex-col py-4 px-3 space-y-1 overflow-y-auto custom-scrollbar">
        <p class="px-3 pt-2 pb-1 text-[10px] font-bold text-slate-500 uppercase tracking-widest">业务</p>
        <router-link to="/order" @click="handleNav('/order')" class="nav-item" active-class="nav-active"><svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg><span>{{ appStore.t('menu_order') }}</span></router-link>
        <router-link to="/mass-order" @click="handleNav('/mass-order')" class="nav-item" active-class="nav-active"><svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 10h16M4 14h16M4 18h16"/></svg><span>{{ appStore.t('menu_mass') }}</span></router-link>
        <router-link to="/history" @click="handleNav('/history')" class="nav-item" active-class="nav-active"><svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg><span>{{ appStore.t('menu_history') }}</span></router-link>

        <p class="px-3 pt-4 pb-1 text-[10px] font-bold text-slate-500 uppercase tracking-widest">资金</p>
        <router-link to="/recharge" @click="handleNav('/recharge')" class="nav-item" active-class="nav-active"><svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/></svg><span>{{ appStore.t('menu_recharge') }}</span></router-link>
        <router-link to="/affiliate" @click="handleNav('/affiliate')" class="nav-item" active-class="nav-active"><svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/></svg><span>{{ appStore.lang === 'zh' ? '推广赚钱' : 'Affiliate' }}</span></router-link>
        <router-link to="/vip" @click="handleNav('/vip')" class="nav-item" active-class="nav-active-vip"><svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z"/></svg><span>{{ appStore.t('menu_vip') }}</span></router-link>

        <template v-if="['admin', 'super_admin', 'agent'].includes(userStore.userInfo?.role)">
          <p class="px-3 pt-4 pb-1 text-[10px] font-bold text-slate-500 uppercase tracking-widest">管理</p>
          <router-link to="/admin" v-if="['admin', 'super_admin'].includes(userStore.userInfo?.role)" @click="handleNav('/admin')" class="nav-item" active-class="nav-active"><svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg><span>{{ appStore.t('menu_admin') }}</span></router-link>
          <router-link to="/admin/backup" v-if="['admin', 'super_admin'].includes(userStore.userInfo?.role)" @click="handleNav('/admin/backup')" class="nav-item" active-class="nav-active"><svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7v8a2 2 0 002 2h6M8 7V5a2 2 0 012-2h4.586a1 1 0 01.707.293l4.414 4.414a1 1 0 01.293.707V15a2 2 0 01-2 2h-2M8 7H6a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2v-2"/></svg><span>{{ appStore.t('menu_backup') || '灾备中心' }}</span></router-link>
          <router-link to="/pricing" v-if="['admin', 'super_admin'].includes(userStore.userInfo?.role)" @click="handleNav('/pricing')" class="nav-item" active-class="nav-active"><svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg><span>分站调价</span></router-link>
          <router-link to="/api-doc" @click="handleNav('/api-doc')" class="nav-item" active-class="nav-active"><svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4"/></svg><span>{{ appStore.t('menu_api') || '开发者 API' }}</span></router-link>
        </template>

        <p class="px-3 pt-4 pb-1 text-[10px] font-bold text-slate-500 uppercase tracking-widest">工具</p>
        <router-link to="/vpn" class="nav-item" active-class="nav-active"><svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg><span>{{ appStore.lang === 'zh' ? '🛡️ 安全节点' : '🛡️ VPN' }}</span></router-link>
      </div>
    </aside>
      ...
      <header class="fixed top-0 left-0 right-0 md:relative md:top-auto md:left-auto md:right-auto h-16 flex-shrink-0 bg-slate-900/60 backdrop-blur-xl border-b border-slate-700/50 flex items-center justify-between px-3 md:px-6 z-[60] w-full md:w-auto shadow-sm">
        ...
        <div class="flex items-center space-x-2 md:space-x-5 text-sm">
          ...
          <div class="flex items-center space-x-1.5 md:space-x-3 text-slate-300">
            ...
            <span v-if="userStore.userInfo?.role === 'super_admin'" class="badge bg-red-500/10 text-red-400 border border-red-500/30 text-[10px] px-2 py-0.5 rounded-full font-bold hidden sm:inline-flex">至尊管理员</span>
            <span v-else-if="userStore.userInfo?.role === 'admin'" class="badge bg-amber-500/10 text-amber-400 border border-amber-500/30 text-[10px] px-2 py-0.5 rounded-full font-bold hidden sm:inline-flex">管理员</span>
            <span v-else-if="userStore.userInfo?.role === 'agent'" class="badge bg-purple-500/10 text-purple-400 border border-purple-500/30 text-[10px] px-2 py-0.5 rounded-full font-bold hidden sm:inline-flex">至尊代理</span>
            <span v-else class="badge bg-slate-500/10 text-slate-400 border border-slate-500/30 text-[10px] px-2 py-0.5 rounded-full font-bold hidden sm:inline-flex">黄金用户</span>
          </div>
          ...
          <div class="relative flex items-center">
            ...
            <div v-show="isProfileMenuOpen" class="absolute right-0 top-full mt-2 w-40 bg-slate-800 border border-slate-700 rounded-xl shadow-xl overflow-hidden animate-fade-in z-[100] py-1">
               <button @click="openProfileModal" class="w-full text-left px-4 py-2.5 text-sm text-slate-300 hover:text-white hover:bg-slate-700 transition flex items-center space-x-2"><svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg><span>修改资料</span></button>
               <button v-if="!['admin', 'super_admin'].includes(userStore.userInfo?.role)" @click="openDeleteModal" class="w-full text-left px-4 py-2.5 text-sm text-slate-300 hover:text-red-400 hover:bg-red-500/10 transition flex items-center space-x-2"><svg class="w-4 h-4 text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg><span>注销账号</span></button>
               <div class="h-px bg-slate-700/50 my-1"></div>
               <button @click="handleLogout" class="w-full text-left px-4 py-2.5 text-sm text-slate-300 hover:text-amber-400 hover:bg-slate-700 transition flex items-center space-x-2"><svg class="w-4 h-4 text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"/></svg><span>退出登录</span></button>
            </div>
          </div>
        </div>
      </header>
...
/* 导航项统一样式 */
.nav-item {
  @apply flex items-center space-x-3 px-3 py-2.5 rounded-lg text-sm font-medium text-slate-400 hover:text-white hover:bg-slate-800/70 transition-all duration-200;
}
.nav-active {
  @apply bg-amber-400/15 text-amber-400 border border-amber-400/30 shadow-sm;
}
.nav-active-vip {
  @apply bg-amber-400/15 text-amber-400 border border-amber-400/30 shadow-sm;
}
...
/* 角色徽章统一样式 */
.badge {
  @apply text-[10px] px-2 py-0.5 rounded-full font-bold;
}
