  ...
  <div class="h-[100dvh] w-full flex bg-slate-950 relative overflow-hidden">
    ...
    <aside :class="['fixed md:relative z-[9999] md:z-50 h-full w-64 bg-slate-900/95 border-r border-slate-800 flex flex-col backdrop-blur-xl transition-transform duration-300 ease-in-out', isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0']">
      ...
      <div class="flex flex-col py-6 px-4 space-y-2 overflow-y-auto custom-scrollbar">
        ...
        <router-link to="/vpn" class="text-slate-500 hover:text-white hover:bg-slate-800 px-4 py-3 rounded-xl flex items-center space-x-3 transition cursor-pointer text-sm"><svg class="w-4 h-4 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"></path></svg><span class="text-emerald-400 text-sm">{{ appStore.lang === 'zh' ? '🛡️ 安全节点' : '🛡️ VPN' }}</span></router-link>
        ...
        <router-link to="/sites" v-if="['admin', 'super_admin', 'agent'].includes(userStore.userInfo?.role)" @click="handleNav('/sites')" active-class="bg-slate-800 text-white" class="text-slate-400 hover:text-white hover:bg-slate-800 px-4 py-3 rounded-xl flex items-center space-x-3 transition cursor-pointer"><svg class="w-5 h-5 text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9"></path></svg><span class="text-indigo-400 font-bold">{{ appStore.lang === 'zh' ? '分站管理' : 'Sub-sites' }}</span></router-link>
