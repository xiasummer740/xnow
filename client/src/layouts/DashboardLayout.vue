      ...
      <header class="fixed top-0 left-0 right-0 md:relative md:top-auto md:left-auto md:right-auto h-16 flex-shrink-0 bg-slate-900/60 backdrop-blur-xl border-b border-slate-700/50 flex items-center justify-between px-3 md:px-6 z-[60] w-full md:w-auto shadow-sm">
        ...
        <div class="flex items-center space-x-2 md:space-x-5 text-sm">
          ...
          <div class="flex items-center space-x-1.5 md:space-x-3 text-slate-300">
            ...
            <span v-if="userStore.userInfo?.role === 'super_admin'" class="badge bg-red-500/10 text-red-400 border border-red-500/30 text-[10px] px-2 py-0.5 rounded-full font-bold inline-flex">{{ appStore.t('super_admin_badge') }}</span>
            <span v-else-if="userStore.userInfo?.role === 'admin'" class="badge bg-amber-500/10 text-amber-400 border border-amber-500/30 text-[10px] px-2 py-0.5 rounded-full font-bold inline-flex">{{ appStore.t('admin_badge') }}</span>
            <span v-else-if="userStore.userInfo?.role === 'agent'" class="badge bg-purple-500/10 text-purple-400 border border-purple-500/30 text-[10px] px-2 py-0.5 rounded-full font-bold inline-flex">{{ appStore.t('agent_badge') }}</span>
            <span v-else class="badge bg-slate-500/10 text-slate-400 border border-slate-500/30 text-[10px] px-2 py-0.5 rounded-full font-bold inline-flex">{{ appStore.t('gold_badge') }}</span>
          </div>
