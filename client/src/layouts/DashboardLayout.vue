<template>
  <div class="min-h-screen flex bg-transparent relative z-10 overflow-hidden">
    ...
    <div class="flex-grow flex flex-col min-w-0 md:ml-0 h-screen relative">
      <header class="h-16 flex-shrink-0 bg-slate-900/80 border-b border-slate-800 flex items-center justify-between px-3 md:px-6 backdrop-blur-sm relative z-[60]">
        ...
        <div class="flex items-center space-x-3 md:space-x-5 text-sm">
          ...
          <div class="flex items-center space-x-1.5 md:space-x-3 text-slate-300">
            ...
            <span v-else class="x-badge badge-gold"><span class="badge-shimmer"></span>{{ appStore.t('gold_badge') || '黄金用户' }}</span>
          </div>
      ...
      <main class="flex-grow p-4 md:p-6 overflow-auto relative z-[50]">
        <router-view :key="$route.fullPath" />
      </main>
      
      <div v-if="profileModal.show" class="fixed inset-0 z-[200] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-fade-in">
        <div class="bg-slate-900 border border-slate-700 p-8 rounded-3xl shadow-[0_20px_60px_rgba(0,0,0,0.8)] max-w-md w-full relative transform transition-all scale-100">
          <button @click="profileModal.show = false" class="absolute top-4 right-4 text-slate-400 hover:text-white"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg></button>
          <h3 class="text-xl font-black text-white mb-6 tracking-wide text-center flex items-center justify-center"><span class="text-blue-400 mr-2">🛡️</span> 安全资料中心</h3>
          ...
          <div class="space-y-4 mb-8">
            ...
            <div><label class="block text-xs font-bold text-slate-400 mb-1">当前密码 (必填)</label><input type="password" v-model="profileModal.form.current_password" class="w-full bg-slate-800 border border-slate-600 rounded-xl px-4 py-3 text-white outline-none focus:border-blue-400 transition font-mono text-sm" placeholder="验证身份"></div>
            
            <div v-if="!userStore.userInfo?.email">
               <label class="block text-xs font-bold text-amber-400 mb-1">您尚未绑定邮箱，请输入一个邮箱接收验证码：</label>
               <input type="email" v-model="profileModal.form.new_email_for_code" class="w-full bg-slate-800 border border-amber-500/50 rounded-xl px-4 py-3 text-white outline-none focus:border-amber-400 transition font-mono text-sm mb-4" placeholder="例如: user@example.com">
            </div>

            <div class="flex space-x-2 items-end">
                <div class="flex-grow"><label class="block text-xs font-bold text-slate-400 mb-1">邮箱验证码 (必填)</label><input type="text" v-model="profileModal.form.email_code" class="w-full bg-slate-800 border border-slate-600 rounded-xl px-4 py-3 text-white outline-none focus:border-blue-400 transition font-mono text-sm" placeholder="6位数字"></div>
                <button @click="sendEmailCode" :disabled="countdown > 0" :class="['w-28 py-3 rounded-xl font-bold transition text-xs border', countdown > 0 ? 'bg-slate-800 text-slate-500 border-slate-700 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-500 text-white border-blue-500 shadow-lg']">{{ countdown > 0 ? countdown + 's 后重发' : '获取验证码' }}</button>
            </div>

            <div class="h-px bg-slate-700/50 my-4"></div>

            <div><label class="block text-xs font-bold text-slate-400 mb-1">新手机号 (不修改请留空)</label><input type="text" v-model="profileModal.form.phone" class="w-full bg-slate-800 border border-slate-600 rounded-xl px-4 py-3 text-white outline-none focus:border-emerald-400 transition font-mono text-sm"></div>
            <div v-if="userStore.userInfo?.email"><label class="block text-xs font-bold text-slate-400 mb-1">新邮箱 (不修改请留空)</label><input type="email" v-model="profileModal.form.email" class="w-full bg-slate-800 border border-slate-600 rounded-xl px-4 py-3 text-white outline-none focus:border-emerald-400 transition font-mono text-sm"></div>
            <div><label class="block text-xs font-bold text-slate-400 mb-1">新密码 (不修改请留空)</label><input type="password" v-model="profileModal.form.new_password" class="w-full bg-slate-800 border border-slate-600 rounded-xl px-4 py-3 text-white outline-none focus:border-emerald-400 transition font-mono text-sm" placeholder="••••••••"></div>
          </div>
          <button @click="submitProfileUpdate" class="w-full font-black text-lg py-3.5 rounded-xl transition transform hover:-translate-y-1 shadow-lg text-white bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-400 hover:to-indigo-500 shadow-[0_0_20px_rgba(59,130,246,0.4)]">执行双重验证并保存</button>
        </div>
      </div>
      
      <div v-if="deleteModal.show" class="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-fade-in">
