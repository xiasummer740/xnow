...
## 触发机制（2026-09-21 生产机上实测坐实，两个方向都验过）

**不需要启动 nginx —— 以 root 跑过哪怕一次配置解析就够了。**

nginx 解析配置时会走到 `ngx_create_paths`，把 `/var/lib/nginx/{proxy,body,fastcgi,uwsgi,scgi}`
的**用户**改成该配置里 `user` 指令指定的那个，**组原样不动**（nginx 传的是 `chown(path, user, -1)`）。
配置里没写 `user` 指令时，nginx 按**编译默认取 `nobody`**。

于是 `nginx -t` / `nginx -T` 这种"只是测一下配置"的只读操作，**实际上会改属主**：

| 配置 | `proxy` | `body` | `fastcgi` |
|---|---|---|---|
| 执行前 | `www-data:root` | `www-data:www-data` | `www-data:www-data` |
| 跑 `nginx -t -c <去掉 user 行的 main 配置>` | **`nobody:root`** | **`nobody:www-data`** | **`nobody:www-data`** |

→ 与故障现场形态（`nobody:root 0700`）**逐字段吻合**，"只有用户变、组不变"这一点也对得上。

⚠️ **仍未坐实的只有「9/20 当天是谁跑的、跑的是哪份配置」** —— 没找到命令行痕迹。
但"这个机制能产生该形态"已从推断升级为**可复现的事实**，不再只是自洽的猜想。

> 📌 **触发条件收窄（当日实测订正）**：能改属主的是「一份**解析得过**、又不含 `user` 指令的
> **main** 配置」。我一度推测 `xnow-spa-https.bak-cache` 那类站点配置是"现成诱饵"——
> **不成立，已推翻**：站点配置顶层放 `server` 块，`nginx -t -c <它>` 第 1 行就报
> `"server" directive is not allowed here`，**解析失败的配置不会走 ngx_create_paths，属主纹丝不动**。
> 所以 9/20 到底是谁跑的哪份配置，**仍未坐实**，别拿站点配置这条去圆它。
>
> 反向的副产品：用**正常的** main 配置跑一次 `nginx -t`，会把属主顺手改回 `www-data`
> —— 属主会被人无声地改来改去（本次排查中复审代理跑 `nginx -T` 就意外"修"了一次，
> 一度干扰了我的对照实验）。**所以巡检判据只看 uid、不看 gid**，gid 本来就允许不一样。
