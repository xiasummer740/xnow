...
## [2026-09-21] 全站下单页「服务加载失败」→ nginx 临时目录属主错位（已修复 + 已杜绝）

### 现象与真凶（真凶不在代码里）

祥哥报障：桌面书签打开站点，下单页服务分类报 `⚠️ 服务加载失败` / `网络异常，服务加载失败，请点击重试`。

`/var/lib/nginx/proxy` 属主是 `nobody:root 0700`，而 nginx worker 跑 `www-data`。
nginx 落盘缓冲失败 → **读到一半放弃上游** → 只把已缓冲的一小截当成功发出去，
`Content-Length` 却仍报完整的 `797760` → 前端 `res.json()` 解析残缺 JSON 抛错 →
落进 `Order.vue` catch 的 `'network'` 分支。**代码里 401/500 的分支设计得好好的，全被吞掉**，
症状极具误导性（把人往查网络/查登录带）。

- 影响约 **27 小时**：`2026-09-20 02:18:06` → `2026-09-21 05:55`，起始点紧跟 9/20 02:11 那次 nginx 改动
- **跨站**：同机 XNOW-Flow（上游 `:8000`）的 `assets/index.js`（1.5MB）同样被腰斩 —— 这是 nginx 层故障，不是本站代码问题
- 定位一行命令：`grep -c "Permission denied) while reading upstream" /var/log/nginx/error.log`
- 修复：`chown -R www-data:www-data /var/lib/nginx/{proxy,body,fastcgi,uwsgi,scgi}`（**不用 reload，立即生效**）
- 祥哥刷新后确认恢复（access log `05:59:27` → 200 + 831274 字节完整响应）

**分层对照（同环境只切一个变量）**：VPS 直连后端完整 → VPS 经本机 nginx 完整 → 经 CF 断裂。
注意第 2 条容易误判成"CF 的问题"：本机 curl 秒收，全程走内存缓冲**不落盘**，所以压根没走到出错那段逻辑。
**「某条路径不复现」≠「这条路径没问题」，要看它有没有走到出问题的逻辑。**

### 祥哥定调「我是需要杜绝这种情况再次发生」→ 三层防复发（提交 `3a3ec91f`）

1. **结构层（最关键）** `/etc/nginx/conf.d/00-proxy-buffer.conf`：`proxy_max_temp_file_size 0`
   —— 响应改同步透传、压根不用临时文件，**腰斩结构上不可能发生**。
   `conf.d/*.conf` 被 include 在 `http` 块内，对所有站点生效。`install.sh` 同步写入，新部署自带免疫。
2. **根因层** `server/src/utils/nginxWatch.js::checkNginxTempDirs`：每 5 分钟查属主，
   不对就 chown 抢修 + 推 TG（顺带把"谁弄坏的"逮现行）。判据**只看 uid 不看 gid** ——
   nginx 传的是 `chown(path, user, -1)`，组本来就允许不一样，比 gid 会天天误报。
3. **感知层** 同文件 `checkNginxErrors`：每 5 分钟巡 error.log 的 `[emerg]/[alert]/[crit]`，
   去噪（SSL 握手扫描噪声 / conflicting server name）后推 TG；游标**按行内容记**，抗日志轮转。

### 实测证据

| 验的是什么 | 怎么验的 | 结果 |
|---|---|---|
| 结构层真的不再落盘 | 看 temp 目录写入痕迹（**不依赖属主状态**） | conf `06:05:45` 生效后，`proxy` 下 0–9 子目录 **mtime 全部冻结在 `05:59:27` 之前**；同期 access.log **8 次** 797760 次次完整。对照修复前：同一客户端、同一接口、同一大小确实产生 temp 写入 + 17 条 crit |
| ~~结构层真能防腰斩~~ | ~~把属主改回故障态后连拉 3 次~~ | **⚠️ 该证据已撤回**：测试窗口内有三次配置解析把属主改回了 www-data（两次为复审代理所跑，pid 在 error.log 对得上），**窗口内属主是否为故障态无法确定**。结论未变，但这个实验不算数 |
| 自愈巡检端到端 | 06:11:37 手动改坏属主 | **06:11:57 自动抢修**（正好撞上定时任务），pm2 日志留下带时间戳记录，5 个目录全部恢复 `www-data:www-data`，**无 TG 发送报错**（Telegram 已接受） |
| 感知层不漏报/不误报 | 伪造日志跑 5 个场景 | 首次只建基线不报；无变化不报；新增 1 条报 1 条；混入噪声报 2 条（噪声不计）；模拟轮转后报 1 条 |

### 触发机制：从「推断」升级为「可复现的事实」（当日实测坐实）

**以 root 跑任何一次 nginx 配置解析**（`nginx -t` / `nginx -T` 即可，**不必启动、不必 reload**）
都会走到 `ngx_create_paths`，把这些目录的**用户**改成该配置里 `user` 指令指定的那个（**组原样不动**）；
配置里没写 `user` 就按编译默认取 **`nobody`**。

实测（生产机，两个方向都验）：

| 配置 | `proxy` | `body` | `fastcgi` |
|---|---|---|---|
| 执行前 | `www-data:root` | `www-data:www-data` | `www-data:www-data` |
| `nginx -t -c <去掉 user 行的 main 配置>` | **`nobody:root`** | **`nobody:www-data`** | **`nobody:www-data`** |

→ 与故障现场（`nobody:root 0700`）**逐字段吻合**。

**⚠️ 当天上午我曾据此把 `xnow-spa-https.bak-cache` 称作"诱饵"，随后实测推翻并订正（提交 `6c66c275`）：**
站点配置顶层放 `server` 块，`nginx -t -c <它>` **第 1 行就报 `"server" directive is not allowed here`**，
属主**纹丝不动** —— 解析失败的配置走不到建目录那一步。
真正的触发条件是「一份**解析得过**、又不含 `user` 指令的 **main** 配置」。
**9/20 究竟是谁跑的哪份配置，仍未坐实**，不再拿站点配置这条去圆它。

> 副产物：用**正常的** main 配置跑一次 `nginx -t` 会把属主顺手改回 `www-data`
> —— 属主会被人无声地改来改去。本次排查中复审代理跑 `nginx -T` 就意外"修"了一次，
> 一度干扰我的对照实验，也正是这一下让机制露了馅。

### 决策副作用 / 代价

- **`proxy_max_temp_file_size 0` 不是白拿的**：响应改同步透传后，慢客户端会**多占用一会儿上游连接**
  （原来 nginx 可以先快速吞完上游、再慢慢喂给客户端）。本项目被代理的最大响应约 1.5MB、并发量小，可接受。
  **这是拿"可能慢一点"换"绝不静默出错"** —— 换得值，但要知道换了什么。
- **`chown -R` 会抹掉历史遗留临时文件的属主线索**：哪天要审计"谁在何时往临时目录写过"，这一维就没了。
- **诱因那半截仍是推断**：现在补了自愈巡检，**它若再发生会留下带时间戳的告警**，等于把推断变成可观测项。

### 同步状态

- 本地 / GitHub / VPS 三者均为 `6c66c275`，VPS 已 `pm2 restart xnow-backend`
- 线上入口包 `index-tUWPgOd3.js` md5 `a3546bb225a121c15ee5ef069f35af7f`，pull 前后三份一致（前端未被动）
- 唯一一次 push 后自检：经 CF 拉 `/api/services/public` → 200 + 797760 完整 + JSON 可解析（2959 条）

### 本次没做 / 仍欠着

- **`xnow-spa-https.bak-cache` 仍在 `sites-enabled/`** —— 风险已按实陈述订正：不是"诱人去 `-c`"，
  而是**同一 `server_name` 出现两次，谁生效看 include 顺序**，顺序一变就由缺 real_ip / 缺缓存头的旧块接管（静默退化）。
  生产块是它的严格超集，移走零功能损失。**已建议祥哥，未擅自动**（不是我建的文件）
- 其余挂起项同 2026-09-20 段末尾

### 独立对抗性复审（立场：证伪「已杜绝」）→ 3 反例 + 5 存疑，逐条处置

**采纳并已修（`3ae208e8`）**

1. **感知层静默漏报（真 bug，我用真实模块复现了）** —— `checkNginxErrors` 初版把
   「还没建基线（`null`）」和「基线是空（`''`）」混为一谈：日志里一条严重错误都没有时会把游标清空，
   下一批 `[crit]` 到达就被当成"首次运行"存成基线、**一条都不报**。
   生产上 logrotate 每天轮转 → 新文件必然无 crit → **当天第一批严重错误必然静默**，
   而"静默故障"正是这道网存在的理由。已修（两态分开，安静窗口保持游标不动），
   9 个场景全部符合预期。
2. **属主巡检只看顶层（漏检）** —— 顶层完全可能被别人的一次正常 `nginx -t` 顺手修回 `www-data`，
   底下被改坏的编号子目录原地留着照样写不进去。已改递归。
   **判别测试**：顶层健康 + 只把 `proxy/0` 改成 `nobody` → **旧代码静默放过、新代码检出并修复**。
3. **"杜绝"措辞过宽** —— 复审用纯 Node 上游（`Content-Length: 797760`、只写 130451 字节后
   `socket.destroy()`）复现出**逐字节相同**的签名（`200 + size_download=130451 + curl rc=18`），
   前端照样落进 `Order.vue` 的 `'network'` 分支。全站"杜绝"收窄为
   「**"nginx 因临时文件落盘失败而腰斩"这条路径消失**」。

**撤回一条我自己的证据** —— 原写的「把属主改回故障态后连拉 3 次 = 3/3 完整」**证据资格不成立**：
那 3 次拉取之后有三次配置解析把属主改回了 `www-data`（其中两次是复审代理自己跑的 `nginx -T`，
pid 在 error.log 里对得上），**测试窗口内属主到底是不是故障态无法确定**。
结论没被推翻，但那个实验不能算数。改用**不依赖属主状态**的运行期证据（子目录 mtime 冻结 + 8 次完整）。
教训：改故障态做对照，**前后必须各留一次 `stat` 时间戳**。

**核实后驳回（存疑，无实证）**

- "按 nginx 源码 EEXIST 分支不该 chown，机制存疑" —— 直接观测优于源码回忆：带内单变量测试，
  **预先存在的目录照样被改**（`33 → 65534 → 33`）。
- "子目录单独错位是盲区" —— 场景成立（已补递归），但**当前生产上不存在**：
  `find /var/lib/nginx -not -uid 33` 只有 `/var/lib/nginx` 自身一条（root 属主是设计如此）。

**记录在案、本次不动**（已进 `ISSUES.md`）：`Order.vue` 把一切解析失败归成"网络异常"；
`sendTgMessage` 是 fire-and-forget 且无 timeout（**"日志里没有 TG 报错" ≠ 消息送达**）；
上传体仍落 `client_body_temp`，属"≤5 分钟自愈"而非"杜绝"。

> 一个查穿了的假象：我一度以为前端那条 `node -e` 卡死是 TG 发不出去，
> 差点得出"告警送不达"的结论。查穿了是 **`Config.findAll` 把 sequelize 连接池拉起来，
> 事件循环有句柄、Node 就不退出** —— 而 `sendTgMessage` 那句 `axios.post` 既没 `return` 也没 `await`，
> 根本挂不住调用方。环境侧也实测过：`fetch` 200/1.2s、`axios` 200/1.3s、`curl` 302/0.47s。

### 收尾（2026-09-21，祥哥点头「修」）

**① 祥哥确认 TG 告警收到了** —— 06:11 那条「nginx 临时目录属主被改坏，已自动抢修」。
这是**感知层最后一跳的唯一缺口**：此前只能证明"环境可达 + 代码未报错"，而 `sendTgMessage`
是 fire-and-forget（Telegram 回 401/400 也不算错），所以"没报错"**推不出**"送达了"。现在有真人确认，闭环。

**② `xnow-spa-https.bak-cache` 已移走**（**移走不是删除**，可回滚）
→ `/root/nginx-stray-20260921/`，与 `/root/nginx-backup-20260920-021106` 放在一起。

动手前**逐行比对**证明生产块是它的严格超集（不能只看行数）：

| 项 | bak-cache | 生产块 xnow-spa-https |
|---|---|---|
| listen / server_name / 证书 / TLS / client_max_body_size | ✅ | ✅ 完全相同 |
| `location /api/` | 6 行 | **逐字一致** |
| real_ip 段 | **0 条** | 23 条 |
| `location /assets/` 长缓存 | ❌ | ✅ |
| `location /` 的 `no-cache` | ❌ | ✅ |
...
- `nginx -t` **不再报 `conflicting server name`**（此前基线 13 条，reload 后仍是 13，无新增）
- reload 后 `set_real_ip_from` 仍 23 条、`real_ip_header` 1 条、`proxy_max_temp_file_size 0` 1 条
- 声明 `xnow.taikon.top` 的 server 从 **2 个降到 1 个**
- 站点：首页 200 / 3441 字节，`/api/services/public` 200 / **797760 完整**
- 缓存头照旧：HTML `no-cache`、assets `public, max-age=31536000, immutable`
- **决定性一条**：经 CF 发请求后**访问日志第一列 = `64.186.242.99`（我的真实出口 IP，不是 CF 边缘 IP）**
  —— 证明移走旧块**没有**破坏 Cloudflare 真实 IP 还原（这是搬移唯一可能伤到的东西）
- 临时目录属主：全树 0 个非 www-data（reload 本身会做一次 ngx_create_paths，属主回到 www-data，属健康态）

**事故线到此结案**：修复 → 三层杜绝 → 独立复审整改 → 隐患源清除 → 感知层送达确认。
